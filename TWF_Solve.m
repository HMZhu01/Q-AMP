function [MSE_X_List, MSE_X1_List, iterations, x_t] = TWF_Solve(Input, Obj)
	% x: N * 1, z: M * 1
	...
	while (diff > diff_tol) && (iterations <= max_iterations)
		...
		In_x = x_t - lr / Phi^(2) * grad;

		Index = abs(In_x) > Th_x;

		x_t = zeros(N, 1);
		x_t(Index) = x_t(Index) - Th_x * sign(x_t(Index));
